/**
 * EpicenterDSP Lite - iOS-only Audio Queue & Library Hook
 *
...
import { MAX_IMPORTED_TRACKS } from "@/config/appEdition";
import { buildContextualPlaybackQueue } from "@/lib/playbackQueue";
...
export function useAudioQueue(): QueueController {
  ...
  const importManualTracksFromNativePicker =
    useCallback(async (): Promise<ImportResult> => {
      if (library.length >= MAX_IMPORTED_TRACKS) {
        return {
          added: 0,
          duplicates: [],
          limitReached: true,
      ...
      try {
        const result = await EpicenterNative.importTracks({
          maxTrackCount: MAX_IMPORTED_TRACKS,
        });
        console.info(
        ...
        return {
          ...
          skippedForLimit:
            typeof result.skippedForLimit === "number"
  ...
  const playNow = useCallback((track: Track, contextTracks?: Track[]) => {
    if (!isValidTrack(track)) return;
    const playbackQueue = buildContextualPlaybackQueue(track, contextTracks);
    setQueue(playbackQueue.tracks);
    setCurrentTrackIndex(playbackQueue.startIndex);
    void setNativeQueue(playbackQueue.tracks, playbackQueue.startIndex);
  }, []);
