...
import { FEATURES, sanitizeLiteEqGains } from "@/config/appEdition";
...
export function useIosNativeAudioProcessor() {
  ...
  // "car" = classic Epicenter (subwoofer). "headphones" = small-driver engine.
  const [epicenterMode, setEpicenterModeState] =
    useState<IOSNativeEpicenterMode>(
      () =>
        (typeof localStorage !== "undefined" &&
          (localStorage.getItem("epicenterMode") as IOSNativeEpicenterMode)) ||
        "car",
    );
  const [eqEnabled, setEqEnabledState] = useState(false);
  const [autoEqEnabled, setAutoEqEnabledState] = useState(false);
  ...
  const applyState = useCallback(
    (state: IOSNativePlaybackState | null | undefined) => {
      ...
      // The direct return value of play()/next()/previous() can be a non-playback
      // acknowledgement such as { status: "ignored" } (a transition was already in
      // flight) or { status: "error" }. Those payloads carry no isPlaying / duration /
      // currentTrack fields, so applying them would wipe the now-playing UI to "nothing
      // playing" in the middle of a skip. Only full playback snapshots (status "ok",
      // emitted via playbackStateChanged / progressChanged / getPlaybackState) should
      // drive UI state here. Errors are surfaced separately through the playbackError
      // listener.
      if (state.status && state.status !== "ok") {
        console.info(
          "[iOS Native Playback] ignored non-ok state",
          state.status,
        );
        return;
      }
  ...
  useEffect(() => {
    ...
    void EpicenterNative.addListener("currentTrackChanged", (event) => {
      ...
      console.info(
        "[Web] currentTrackChanged received requestId=",
        event.requestId,
        "trackId=",
        event.track.id,
      );
      console.info(
        "[Bridge] event currentTrackChanged requestId=",
        event.requestId,
        "trackId=",
        event.track.id,
      );
  ...
  const next = useCallback(
    async (requestId?: string) => {
      ...
      const nativeRequestId =
        requestId ?? `processor-next-${transitionRequestId}`;
  ...
  const previous = useCallback(
    async (requestId?: string) => {
      ...
      const nativeRequestId =
        requestId ?? `processor-previous-${transitionRequestId}`;
  ...
  const setEpicenterMode = useCallback(
    (mode: IOSNativeEpicenterMode) => {
      setEpicenterModeState(mode);
      if (typeof localStorage !== "undefined")
        localStorage.setItem("epicenterMode", mode);
      void EpicenterNative.setEpicenterMode({ mode }).catch(reportError);
    },
    [reportError],
  );

  // Push the remembered mode to the native engine on startup.
  useEffect(() => {
    void EpicenterNative.setEpicenterMode({ mode: epicenterMode }).catch(
      reportError,
    );
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  // Auto-EQ: native analyzes each song and applies a bespoke 31-band curve. When enabled the
  // native side re-runs per track on load; the resulting bands arrive through the normal
  // playbackStateChanged event, so the UI sliders update on their own.
  const setAutoEqEnabled = useCallback(
    (enabled: boolean) => {
      if (!FEATURES.autoEq) {
        setAutoEqEnabledState(false);
        void EpicenterNative.setAutoEqEnabled({ enabled: false }).catch(
          reportError,
        );
        return;
      }
      setAutoEqEnabledState(enabled);
      if (enabled) setEqEnabledState(true);
      void EpicenterNative.setAutoEqEnabled({ enabled }).catch(reportError);
    },
    [reportError],
  );
  ...
  const setReverbEnabled = useCallback(
    (enabled: boolean) => {
      if (!FEATURES.spatialEffects) return;
  ...
  const setEqPresetGains = useCallback(
    (gains: number[]) => {
      const safeGains = sanitizeLiteEqGains(gains);
      setEqBands((previous) =>
        previous.map((band, index) => ({
          ...band,
          gain: safeGains[index] ?? 0,
        })),
      );
      setEqEnabledState(true);
      void EpicenterNative.setEqEnabled({ enabled: true }).catch(reportError);
      void EpicenterNative.setEqBands({ gains: safeGains }).catch(reportError);
    },
    [reportError],
  );
  ...
  useEffect(() => {
    ...
    if (!FEATURES.spatialEffects) {
      void EpicenterNative.setReverbEnabled({ enabled: false }).catch(
        reportError,
      );
      void EpicenterNative.setConcertHallEnabled({ enabled: false }).catch(
        reportError,
      );
    }
  }, [reportError]);
