...
export function isLosslessAudioFormat(
  codecOrExtension?: string | null,
  fileExtension?: string | null,
): boolean {
  const tokens = [
    normalizeToken(codecOrExtension),
    normalizeToken(fileExtension),
  ].filter(Boolean) as string[];
  return tokens.some(
    (token) => LOSSLESS_CODECS.has(token) || LOSSLESS_EXTENSIONS.has(token),
  );
}

export function isLossyAudioFormat(
  codecOrExtension?: string | null,
  fileExtension?: string | null,
): boolean {
...
export function isHiResQuality(
  ...
  if (
    ...
    return (
      bitDepth >= HI_RES_MIN_BIT_DEPTH &&
      sampleRate >= HI_RES_MIN_SAMPLE_RATE
    );
  }

  if (sampleRate < HI_RES_FALLBACK_SAMPLE_RATE) return false;

  const codec = normalizeToken(codecOrExtension);
  if (codec && LOSSY_CODECS.has(codec)) return false;
  if (codec && LOSSLESS_CODECS.has(codec)) return true;

  return (
    isLosslessAudioFormat(undefined, fileExtension) &&
    !isLossyAudioFormat(undefined, fileExtension)
  );
}

export function isCdQuality(
  bitDepth?: number,
  sampleRate?: number,
): boolean {
  return (
    bitDepth === CD_QUALITY_BIT_DEPTH &&
    sampleRate === CD_QUALITY_SAMPLE_RATE
  );
}
...
export function qualityTier(
  ...
  if (
    (bitDepth ?? 0) >= 32 &&
    (sampleRate ?? 0) >= HI_RES_MIN_SAMPLE_RATE &&
    isLosslessAudioFormat(codecOrExtension, fileExtension)
  ) {
    return "studio";
  }
  if (
  ...
  if (
    isLossyAudioFormat(codecOrExtension, fileExtension) ||
    (typeof bitrate === "number" && bitrate > 0 && !bitDepth)
  ) {
    return "lossy";
  }
...
export function formatQualityLabel(
  ...
  const parts: string[] = [];
  if (typeof bitDepth === "number") parts.push(`${bitDepth}-bit`);
  if (typeof sampleRate === "number") {
    const sampleRateKHz = (sampleRate / 1000).toFixed(
