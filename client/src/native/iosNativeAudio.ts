...
export interface IOSNativeAdConsentState {
  status: IOSNativeAudioStatus;
...
export type IOSNativeEpicenterMode = "car" | "headphones";
...
export interface IOSNativeAudioInterruptionEvent {
  phase: "began" | "ended";
}

export interface IOSNativeAutoEqAppliedEvent {
  status: string;
  stableId: string;
  cached: boolean;
  gains: number[];
...
export interface EpicenterNativePlugin {
  ...
  }): Promise<IOSNativeImportTracksResult>;
  getAdConsentState(): Promise<IOSNativeAdConsentState>;
  showAdPrivacyOptions(): Promise<{ status: IOSNativeAudioStatus }>;
  getLibraryPage(
  ...
  setAutoEqEnabled(params: {
    enabled: boolean;
  }): Promise<{ status: string; autoEqEnabled: boolean }>;
  ...
  addListener(
    ...
    listenerFunc: (event: IOSNativeAudioInterruptionEvent) => void,
  ): Promise<PluginListenerHandle>;
  addListener(
    eventName: "autoEqApplied",
    listenerFunc: (event: IOSNativeAutoEqAppliedEvent) => void,
  ): Promise<PluginListenerHandle>;
