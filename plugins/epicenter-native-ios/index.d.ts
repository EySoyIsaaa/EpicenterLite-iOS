...
export interface EpicenterNativeIosPlugin {
  importTracks(params?: { maxTrackCount?: number }): Promise<{
    status: string;
  ...
  getAdConsentState(): Promise<{
    status: string;
    canRequestAds: boolean;
    privacyOptionsRequired: boolean;
  }>;
  showAdPrivacyOptions(): Promise<{ status: string }>;
  setEpicenterEnabled(params: {
    enabled: boolean;
  }): Promise<{ status: string; epicenter: EpicenterState }>;
  setEpicenterParams(
    params: Partial<Omit<EpicenterState, "enabled">> & {
      sweep?: number;
      output?: number;
    },
  ): Promise<{ status: string; epicenter: EpicenterState }>;
  ...
  setEqPreset(params: {
    name?: string;
    gains: number[];
  }): Promise<EqState & { status: string; preset?: string | null }>;
  resetEq(): Promise<EqState & { status: string }>;
  setReverbEnabled(params: {
    enabled: boolean;
  }): Promise<FxState & { status: string }>;
  setReverbAmount(params: {
    amount: number;
  }): Promise<FxState & { status: string }>;
