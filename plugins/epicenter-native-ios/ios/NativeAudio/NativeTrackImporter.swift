...
struct NativeImportBatchResult {
...
final class NativeTrackImporter: NSObject, UIDocumentPickerDelegate {
    ...
    private var completion: ((Result<NativeImportBatchResult, Error>) -> Void)?
    private var maximumTrackCount: Int?
    ...
    func importTracks(
        ...
        completion: @escaping (Result<NativeImportBatchResult, Error>) -> Void
    ) {
        self.completion = completion
        self.maximumTrackCount = maximumTrackCount.flatMap { $0 > 0 ? $0 : nil }
    ...
    func documentPickerWasCancelled(_ controller: UIDocumentPickerViewController) {
        completion?(.success(NativeImportBatchResult(
            tracks: [],
            duplicates: [],
            skippedForLimit: 0
        )))
        completion = nil
        maximumTrackCount = nil
    }
    ...
    func documentPicker(_ controller: UIDocumentPickerViewController, didPickDocumentsAt urls: [URL]) {
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            ...
            var storedTrackCount = self.repository.trackCount()

            for url in urls {
                do {
                    let track = try self.importSingleTrack(from: url)

                    if let existingTrack = self.repository.findTrack(id: track.stableId) {
                        self.removeDuplicateSandboxCopyIfNeeded(
                            importedTrack: track,
                            savedTrack: existingTrack
                        )
                        duplicateFileNames.append(url.lastPathComponent)
                        continue
                    }

                    if let maximumTrackCount = self.maximumTrackCount,
                       storedTrackCount >= maximumTrackCount {
                        self.removeUnsavedSandboxFiles(track)
                        skippedForLimit += 1
                        continue
                    }
            ...
            DispatchQueue.main.async { [weak self] in
                self?.completion?(.success(NativeImportBatchResult(
                    tracks: importedTracks,
                    duplicates: duplicateFileNames,
                    skippedForLimit: skippedForLimit
                )))
                self?.completion = nil
                self?.maximumTrackCount = nil
            }
        }
    }
    ...
    private func removeUnsavedSandboxFiles(_ track: NativeTrack) {
        if let localFilePath = track.localFilePath {
            try? FileManager.default.removeItem(atPath: localFilePath)
        }
        if let albumArtUri = track.albumArtUri {
            try? FileManager.default.removeItem(atPath: albumArtUri)
        }
        if let optimizedUrl = track.optimizedUrl {
            try? FileManager.default.removeItem(atPath: optimizedUrl)
        }
    }
