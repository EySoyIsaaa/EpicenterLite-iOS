...
final class NativeAudioEngine {
    ...
    /// Read-only snapshot of the currently decoded track, for offline analysis (Auto-EQ).
    /// The buffer is a reference type that stays alive as long as the caller holds it, so it is
    /// safe to analyze off-thread even if a later load reassigns `audioBuffer`.
    func decodedTrackSnapshot() -> (buffer: AVAudioPCMBuffer, sampleRate: Double, stableId: String)? {
        guard let buffer = audioBuffer,
              let format = audioFormat,
              let track = loadedTrack else {
            return nil
        }
        return (buffer, format.sampleRate, track.stableId)
    }
    ...
    func load(track: NativeTrack, startAt seconds: Double = 0) throws {
        ...
        scheduleToken += 1
        // Fully stop (not just pause) before rebuilding the source-node graph in
        // loadFullBufferForDSP -> configureSourceNode. After only pause(), detaching and
    ...
    func setEpicenterMode(headphones: Bool) -> [String: Any] {
        epicenterDSP.setHeadphonesMode(headphones)
        print("[iOS Epicenter DSP] mode \(headphones ? "headphones" : "car")")
        return ["status": "ok", "epicenter": epicenterDSP.stateDictionary()]
    }
