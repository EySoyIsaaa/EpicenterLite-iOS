...
final class NativePlaybackController {
    /// Single shared controller keeps bridge reloads from creating multiple engines
    /// that would fight over the same audio session.
    ...
    private let queue = DispatchQueue(label: "com.epicenterdsp.lite.native-playback-controller")
    ...
    private let analysisQueue = DispatchQueue(label: "com.epicenterdsp.lite.auto-eq", qos: .utility)
    ...
    func setEpicenterMode(headphones: Bool) -> [String: Any] {
        queue.sync {
            let response = engine.setEpicenterMode(headphones: headphones)
            emit("playbackStateChanged", engine.playbackState(queue: queueManager.dictionary))
            return response
        }
    }
    ...
    func setAutoEqEnabled(_ enabled: Bool) -> [String: Any] {
        queue.sync {
            autoEqEnabled = enabled
            print("[AutoEQ] enabled=\(enabled)")
            if enabled {
                triggerAutoEQIfEnabled()
            }
            return ["status": "ok", "autoEqEnabled": enabled]
        }
    }
    ...
    private func loadTrack(_ track: NativeTrack, at index: Int, requestId: String, shouldRestartLoadedTrack: Bool, skipOnFailure: Bool, replaceQueueOnSuccess: [String]? = nil) -> [String: Any] {
        ...
        do {
            ...
            // Actually start playback. Without this, load() leaves the engine paused
    ...
    // MARK: - Auto-EQ (per-song analysis)
    // Must be called on `queue`. Applies a cached curve immediately, otherwise analyzes the
    // decoded buffer off-thread and applies when done (if the same track is still loaded).
    private func triggerAutoEQIfEnabled() {
        guard autoEqEnabled, let snapshot = engine.decodedTrackSnapshot() else { return }
        let stableId = snapshot.stableId

        if let cached = AutoEQStore.shared.gains(forStableId: stableId) {
            applyAutoEQ(gains: cached, stableId: stableId, cached: true)
            return
        }

        analysisQueue.async { [weak self] in
            guard let self = self else { return }
            guard let gains = EpicenterAutoEQAnalyzer.computeGains(
                buffer: snapshot.buffer,
                sampleRate: snapshot.sampleRate
            ) else {
                print("[AutoEQ] analysis skipped (track too short/unreadable) stableId=\(stableId)")
                return
            }
            AutoEQStore.shared.save(gains: gains, forStableId: stableId)
            self.queue.async {
                // Only apply if the same track is still loaded (user may have skipped).
                guard let current = self.engine.decodedTrackSnapshot(), current.stableId == stableId else {
                    return
                }
                self.applyAutoEQ(gains: gains, stableId: stableId, cached: false)
            }
        }
    }
    ...
    private func handleInterruptionBegan() {
        queue.async { [weak self] in
            ...
            self.emit("audioInterruption", ["phase": "began"])
            self.emit("playbackStateChanged", state)
        }
    }
    ...
    private func handleInterruptionEnded() {
        queue.async { [weak self] in
            ...
            self.emit("audioInterruption", ["phase": "ended"])
            self.emit("playbackStateChanged", state)
        }
    }
    ...
    private func configureRemoteCommands() {
        _ = remoteCommandManager.configure(
            handlers: RemoteCommandManager.Handlers(
                ...
                next: { [weak self] requestId in self?.isAcceptableRemoteResponse(self?.next(source: "remote", requestId: requestId)) == true },
                previous: { [weak self] requestId in self?.isAcceptableRemoteResponse(self?.previous(source: "remote", requestId: requestId)) == true },
    ...
    /// A remote (lock screen / Control Center) command is considered handled when the
    /// controller either succeeded or deliberately ignored it because a transition is
    /// already in flight. Reporting `.commandFailed`/`.noSuchContent` for a transient
    /// "ignored" makes the system controls flicker during rapid skips.
    private func isAcceptableRemoteResponse(_ response: [String: Any]?) -> Bool {
